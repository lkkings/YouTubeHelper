...
class Profile:
            ...
            async with session.request(method, url, headers=headers, data=data, timeout=10) as response:
                if response.status == 200:
                    if response.text != '':
                        ...
                        raise RuntimeError(
                            '获取接口数据失败，请从删除配置文件中的cookie，重新扫码登录并检查是否触发人机验证\r')
                else:
    ...
    async def get_diff_type_url(self, config: dict, sec_user_id: Util.Union[str, None], count=20, cursor=0) -> str:
        ...
        if mode == "post" and sec_user_id is not None:
            params = XB.getXBogus(
                f'aid=6383&sec_user_id={sec_user_id}&count={count}&max_cursor={cursor}&cookie_enabled=true&platform=PC&downlink=10')
    ...
    async def get_user_profile_info(self, headers: dict, sec_user_id: str) -> dict:
        ...
        params = XB.getXBogus(
            f"device_platform=webapp&aid=6383&sec_user_id={sec_user_id}&cookie_enabled=true&platform=PC&downlink=10")
                ...
                async with session.get(url=domain + params[0],
                    ...
                    if response.status == 200:
                        if response.text != '':
                            ...
                            raise RuntimeError(
                                "接口返回空，检查cookie是否过期以及是否出现人机验证，解决不了请重新扫码登录\r")
                    else:
    ...
    async def get_user_post_info(self, headers: dict, url: str) -> dict:
        ...
        try:
            if self.config['mode'] != 'collection':
        ...
        if aweme_list == []:
            ...
            for item in aweme_list:
                ...
                # 将UNIX时间戳转换为格式化的字符串
                data['create_time'] = Util.time.strftime('%Y-%m-%d %H.%M.%S',
                                                         Util.time.localtime(item.get("create_time", None)))

                # 作者相关
                data['uid'] = author.get("uid", None)
                data['sec_uid'] = author['sec_uid']
                # 判断昵称映射表中是否已经存在该用户
                if DB.nickMapper.get_nickname(author['sec_uid']) is None:
                    # 如果不存在，添加昵称映射
                    DB.nickMapper.add_mapping(author['sec_uid'], author['nickname'])
                    # 关闭索引
                    # DB.nickMapper.close()
                # 获取昵称映射
                data['nickname'] = DB.nickMapper.get_nickname(author['sec_uid'])
    ...
    async def get_Profile(self, count: int = 20) -> None:
        ...
        如果 mode 是 'collection'，则 params 将不包含 sec_user_id，否则包含 sec_user_id。
        ...
        try:
            ...
            # 判断昵称映射表中是否已经存在该用户
            if DB.nickMapper.get_nickname(self.sec_user_id) is None:
                # 如果不存在，添加昵称映射
                DB.nickMapper.add_mapping(self.sec_user_id, self.nickname)
            # 根据映射表中的唯一标识获取用户的昵称，即使用户修改昵称也不会影响文件目录
            self.nickname = DB.nickMapper.get_nickname(self.sec_user_id)
